...
public abstract class Header2FooterRcvAdapter<T> extends  BaseQuickRcvAdapter<T>{
    ...
    public ViewHolderWithRecHelper onCreateViewHolder(final ViewGroup parent, int viewType) {
        if (viewType <=HEADER_TYPE&&viewType > HEADER_TYPE-1000){
            ...
            QuickConfig.e("step 6:" + viewType + " \t HEADER_TYPE-1000" + (HEADER_TYPE - 1000) + " \t HEADER_TYPE" + HEADER_TYPE);
    ...
    public void relatedList(Object listView) {
        if (listView instanceof RecyclerView){
            ...
            /**
             *  When you add a footerview, you also need to call this because
             *  I am the one that has been added to remove the process
             */
            gridSetLookup();
    ...
    @Override
    public void aniException() {
        super.aniException();
        if (mHeaderViews.size()>0||mFooterViews.size()>0) {
            throw new IllegalStateException("have header or footer not use animation ");
        }
    }
}
